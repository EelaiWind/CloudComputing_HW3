    ...
    def main(args: Array[String]) {
        ...
        while(true){
            ...
            graphStructure = graphStructure.flatMap( data => {
                ...
                List( (data._1, (value._1,0.0,links) ) ):::links.map( (_, (0.0,distributedScore, List[String]())) )
            })
        ...
        // === Sort Result ===
        graphStructure = graphStructure.sortBy( data => (-1*data._2._2, data._1) )
        graphStructure.cache
        graphStructure.map( data => data._1+"\t"+data._2._2 ).saveAsTextFile(outputPath)
        sc.stop()
    }
